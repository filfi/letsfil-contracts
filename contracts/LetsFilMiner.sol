...
contract LetsFilMiner {
    ...
    function spSignWithMiner(uint64 minerId) public {
        ...
        MinerTypes.GetBeneficiaryReturn memory bene = MinerAPI.getBeneficiary(CommonTypes.FilActorId.wrap(minerId));
        require(keccak256(bene.active.beneficiary.data) == keccak256(ownerNew.data), "LetsFilMiner: owner is not beneficiary.");
    }
    ...
    function changeOwner(uint64 minerId, CommonTypes.FilAddress memory addr) public {
        require(FilAddresses.validate(addr), "LetsFilMiner: address is invalid.");
    ...
    function sendWithValue(uint64 minerId, uint256 amount) public payable {
        uint256 beforeSend = address(this).balance;
        SendAPI.send(CommonTypes.FilActorId.wrap(minerId), amount);
        require(beforeSend - address(this).balance == amount, "LetsFilMiner: send failure.");
    }
